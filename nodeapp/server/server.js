...
// configure logging
app.use(
  log4js.connectLogger(dslog.getLogger(), {
    level: "auto",
    format:
      ":remote-addr - :method :url :status - :user-agent - :content-length :response-time ms"
  })
);
...
//start app
app.listen(
  process.env.PORT || serverConfigs.runTimeConfig[global.env].port,
  null
);
dslog.info(
  "Running in " +
    global.env +
    " mode @ " +
    serverConfigs.runTimeConfig[global.env].uri +
    ":" +
    serverConfigs.runTimeConfig[global.env].port
);
console.log(
  "Running in " +
    global.env +
